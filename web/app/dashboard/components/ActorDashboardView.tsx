/**
 * ActorDashboardView - Centro de Operaciones de FoodTrace.
 * 
 * Este componente dinámico adapta su interfaz según el rol del usuario conectado
 * para permitir la creación, visualización y transferencia de lotes de productos.
 * 
 * @component
 * @param {string} role - El rol del actor (producer, factory, retailer, consumer).
 * @param {any} program - Instancia del programa Anchor para interactuar con Solana.
 * @param {PublicKey | null} publicKey - Dirección pública de la wallet del usuario.
 * 
 * @description
 * - Gestión de Lotes: Permite crear "Batches" (lotes) y vincularlos a insumos previos (parent_sources).
 * - Sistema de Transferencias: Maneja solicitudes de envío y recepción entre diferentes actores.
 * - Enriquecimiento de Datos: Cruza información de transferencias con datos de lotes para mostrar nombres de productos legibles.
 * - Navegación: Sistema de vistas internas ('menu', 'create', 'list', 'transfer', 'incoming').
 */
...
export const ActorDashboardView = ({
  ...
  // Añade estos estados nuevos dentro de ActorDashboardView
  ...
  const fetchIncomingTransfers = async () => {
    ...
    try {
      ...
      // Obtener todas las transferencias dirigidas 
      ...
      // "Enriquecer" los datos buscando el nombre del producto en cada Batch
  ...
  const handleCreateSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Preparamos el array de fuentes (parent_sources)
      ...
      // Llamada al método actualizado en el Smart Contract
  ...
  // Función para enviar la transferencia a la blockchain
  const handleTransferSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // PDA del receptor (A quien se envía)
      const [recipientProfilePDA] = PublicKey.findProgramAddressSync(
        [Buffer.from("actor"), recipientPubKey.toBuffer()],
        program.programId
      );

      // Llamada explícita (pasa todas las cuentas para evitar el auto-resolution)
  ...
  const handleAcceptTransfer = async (transfer: any) => {
    ...
    setLoading(true);
    try {
      // Calculamos la PDA del Batch original para poder actualizarlo
      ...
      // Llamada a la instrucción accept_transfer en Rust
      const tx = await program.methods
        .acceptTransfer()
        .accounts({
  ...
  const fetchAcceptedTransfers = async () => {
    ...
    try {
      setLoading(true);
      // Buscamos las transferencias donde yo soy el destinatario
      const incoming = await program.account.transferRequest.all([
        {
          memcmp: {
      ...
      // Filtramos solo las que ya fueron ACEPTADAS
      const accepted = incoming.filter((t: any) => t.account.status.accepted !== undefined);

      // Buscamos el nombre del producto en el Batch original de cada transferencia
                  ...
                  <div className="space-y-2 max-h-60 overflow-y-auto pr-2 custom-scrollbar">
                    {acceptedTransfers.map((transfer) => (
                      <div
                        ...
                        className={`p-4 rounded-xl border transition-all flex justify-between items-center ${selectedSources.includes(transfer.publicKey.toBase58())
                      ...
                      >
                        ...
                        <div className={`w-5 h-5 rounded-full border flex items-center justify-center transition-all ${selectedSources.includes(transfer.publicKey.toBase58())
